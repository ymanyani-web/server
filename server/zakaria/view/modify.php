  ...
  #sup{
	...
	<div class="wrapper">
			<!-- banner -->
					...
					<div class="langue">
						<a href="../view/modify.php?lg=1">عربي | </a>
						<a href="../view/modify.php">Français</a>
					</div>
			</div>
...
$reponse = $bdd->query("SELECT * FROM client WHERE id = $iid");
...
<div class="login-box">
	...
	<div class="user-box">
		<input list="nom22" name="nom" id="nom" value ="<?php echo $r333['nom']?>" required>
		<label>Nom et prenom</label>
    </div>

	<div class="user-box" >
		<input type="text" name="cin" id="username-m" value ="<?php echo $r333['cin']?>" required>
		<label>C.I.N</label>
	</div>

	<div class="user-box">
		<input type="text" name="libelle" value="<?php echo $donnees['libelle']?>">
		<label>libelle</label>
	</div>
	<?php if($donnees['debit'] != 0){ ?>
		<div class="user-box">
			<input type="number" id="debit" name="debit" value="<?php echo $donnees['debit']?>" required>
			<label for="debit">debit</label>
		</div>		
	<?php } ?>
	<?php if($donnees['credit'] != 0){ ?>
		<div class="user-box">
			<input type="text" id="credit" name="credit" value="<?php echo $donnees['credit']?>" required>
			<label >credit</label>
		</div>
	<?php } ?>
	...
	<div id="inp">
		...
		<input type="button" value="Suprimer" id="sup" onclick="document.getElementById('id01').style.display = 'block'">
	</div>
</div>

<div id="id01" class="modal">
  <form class="modal-content animate" action="modify.php?id=<?php echo $iid?>" method="post">
...
if(!empty($_POST['pswd']))
{
	if($_POST['pswd'] == '123456'){
		?>
		<script>location.replace("../controller/delete.php?id=<?php echo $iid?>");</script>
		<?php
	}
	else{
		?>
